...
The **Image to 3D** node family converts a single image into a fully generated **3D mesh**.  
...
- include **depth cues** such as shadows and occlusion  
...
- **Disable PBR** → geometry only  
- **Disable Texture** → unlit mesh output  
...
2. Add a short text label if needed (optional)  
3. Select backend model  
4. Click **Run**
...
| Feature | Benefit |
|--------|---------|
| Pre-isolated image | Most accurate geometry |
| Neutral background | Reduces artifacts |
| Depth / shadows | Helps model infer volume |
| Perspective view | Better reconstruction |
| Multiple backends | Low-poly, high-detail, quad/tri options |
| PBR toggle | Mesh-only or full texture output |

The **Image to 3D Nodes** turn your isolated 2D images into ready-to-use mesh assets that can immediately enter production workflows across Blender, Unity, Unreal, and procedural pipelines.
