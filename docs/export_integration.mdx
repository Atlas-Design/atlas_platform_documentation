...
- **`.blend` files**  
  ...
  - **Exporting as FBX:**  
    You can directly export your model in **FBX (.fbx) format** from Blender via **File → Export → FBX**.

  - **Ensuring Separate Textures:**  
    To extract textures as individual files, **unpack them** from the `.blend` file. This will generate a folder containing the texture files next to your scene file, ensuring they are accessible separately.
